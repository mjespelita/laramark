...
<html lang='{{ str_replace('_', '-', app()->getLocale()) }}'>
    ...
    <body class='font-sans antialiased'>
        ...
        <div class='content'>
            ...
            <!-- Create Group Modal -->
            <div class="modal fade" id="createNewGroup" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="" method="post">
                            ...
                            <div class="form-group mt-4">
                                <button class="create-group-btn btn btn-success form-control">Create</button>
                            </div>
                        </form>
                    </div>
            ...
            @foreach (App\Models\Chats::all() as $chat)
                <!-- Add Member Modal -->
                <div class="modal fade add-members-modal" id="addMember{{ $chat->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">

                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Add New Members to {{ $chat->name }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        ...
                        <div class="modal-body">
                            <form action="" method="post">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control users-search-input" placeholder="Search User...">
                                    <div style="height: 300px; overflow-y: scroll;" class="py-2">
                                        @foreach (App\Models\User::whereNot('name', 'Group Chat')->whereNot('id', Auth::user()->id)->get() as $user)
                                            <div class="users mb-3" style="display: flex; align-items: center; gap: 10px;">
                                                @php
                                                ...
                                                <input type="checkbox" class="member" data-id="{{ $user->id }}">
                                ...
                                <div class="form-group mt-4">
                                    <button class="add-member-btn btn btn-success form-control" data-id="{{ $chat->id }}">Add Member</button>
                                </div>
                            </form>
                        </div>
                ...
                <!-- show Member Modal -->
                <div class="modal fade remove-members-modal" id="showMembers{{ $chat->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">

                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Members of {{ $chat->name }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        ...
                        <div class="modal-body">
                            <form action="" method="post">
                                <div class="form-group">
                                    <input type="text" class="form-control users-search-input" placeholder="Search User...">
                                    <div style="height: 300px; overflow-y: scroll;" class="py-2">
                                        @foreach (App\Models\Chatparticipants::where('chats_id', $chat->id)->get() as $user)
                                            @if ($user->users_id != 4)

                                            <div class="users mb-3" style="display: flex; align-items: center; gap: 10px;">
                                                @php
                                                    $hasPhoto = App\Models\User::where('id', $user->users_id)->value('profile_photo_path') ?? "";
                                                    $initials = '';
                                                    if (!$hasPhoto) {
                                                        $names = explode(' ', trim(App\Models\User::where('id', $user->users_id)->value('name') ?? ""));
                                                        $initials = strtoupper(substr($names[0], 0, 1) . substr(end($names), 0, 1));
                                                    }
                                                @endphp

                                                <input type="checkbox" class="member" data-id="{{ $user->users_id ?? "" }}">

                                                @if ($hasPhoto)
                                                    <img src="{{ url('storage/' . App\Models\User::where('id', $user->users_id)->value('profile_photo_path') ?? "") }}"
                                                        alt="{{ App\Models\User::where('id', $user->users_id)->value('name') ?? "" }}"
                                                        style="width: 36px; height: 36px; border-radius: 50%; object-fit: cover;">
                                                @else
                                                    <div style="
                                                        width: 36px;
                                                ...
                                                @endif {{ App\Models\User::where('id', $user->users_id)->value('name') ?? "" }}
                                            </div>
                                            @endif
                                        @endforeach
                                    </div>
                                ...
                                <div class="form-group mt-4">
                                    <button class="remove-member-btn btn btn-success form-control" data-id="{{ $chat->id }}">Remove Member</button>
                                </div>
                            </form>
                        </div>
        ...
        <script>
            $(document).ready(function () {
                ...
                $(".users-search-input").on("keyup", function() {
                ...
                $(document).on('click', '.create-group-btn', function (e) {
                    ...
                    $.post('/create-group', {
                        ...
                        $('.groupName').val("")
                        // Close the modal
                        let modalEl = document.getElementById('createNewGroup');
